const Service = require('../../models/queue');
...
const outputDIR = '.'
var tableId = 'ContentPlaceHolder1_Content_lstQueues';
var columns = 7;
var pageSize = 8;
  ...
  switch(resource) {
      ...
      case 'Queues':
          $('tr', '#' + tableId).each(function () {
            ...
            if (tds.length === columns) {
                let obj = {};
                obj.queueName = formatText($(tds[0]).text());
                obj.queueType = formatText($(tds[1]).text());
                obj.location = formatText($(tds[2]).text());
